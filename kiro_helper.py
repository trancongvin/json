...
# --- AWS IAM Identity Center (awsidc / AWS SSO OIDC) --------------------------
# Kiro portal chuyen callback dang /signin/callback?login_option=awsidc&issuer_url=
# https://d-XXXX.awsapps.com/ (KHONG co client_id). Client phai tu RegisterClient
# tren oidc.{region}.amazonaws.com roi chay auth-code + PKCE. Token tra ve dung
# TRUC TIEP lam Bearer (KHONG gui TokenType, khac external_idp).
AWSIDC_OIDC_HOST = "https://oidc.%s.amazonaws.com"      # % region
AWSIDC_CLIENT_NAME = "KiroIDE"
# AWS SSO OIDC bat buoc redirect URI dung loopback IP (127.0.0.1), KHONG chap nhan
# ten "localhost" (bao invalid_redirect_uri). Listener da bind ca 127.0.0.1 nen
# code van ve dung. Dung bien the nay rieng cho leg awsidc.
AWSIDC_REDIRECT_URI = "http://127.0.0.1:%d%s" % (SOCIAL_REDIRECT_PORT, OAUTH_CALLBACK_PATH)
# Scope carried over tu amazon-q-developer-cli Builder ID flow — CAN verify live.
AWSIDC_SCOPES = ["codewhisperer:completions", "codewhisperer:analysis",
                 "codewhisperer:conversations"]
# Host issuer IAM Identity Center hop le (chan SSRF): phai la *.awsapps.com.
ALLOWED_AWSIDC_ISSUER_SUFFIX = ".awsapps.com"
...
# http_get_text fetches url and returns the decoded body text (dung de scrape env
# blob cua trang start IAM Identity Center khi phai suy region).
def http_get_text(url, proxy_url, follow_redirects=True, timeout=30):
...
# --- AWS IAM Identity Center (awsidc) leg -------------------------------------

# validate_awsidc_issuer bao dam issuer_url la https, host co ten (khong phai IP)
# va ket thuc bang .awsapps.com. issuer den tu callback co the bi tac dong nen phai
# rang buoc (SSRF/open-redirect guard), giong validate_external_idp_endpoint.
def validate_awsidc_issuer(raw_url):
    parsed = urllib.parse.urlparse((raw_url or "").strip())
    if parsed.scheme.lower() != "https":
        raise ValueError("awsidc issuer phai https: %r" % raw_url)
    host = (parsed.hostname or "").lower()
    if not host:
        raise ValueError("awsidc issuer khong co host: %r" % raw_url)
    try:
        socket.inet_pton(socket.AF_INET, host); is_ip = True
    except OSError:
        try:
            socket.inet_pton(socket.AF_INET6, host); is_ip = True
        except OSError:
            is_ip = False
    if is_ip:
        raise ValueError("awsidc issuer khong duoc la IP: %r" % host)
    if not host.endswith(ALLOWED_AWSIDC_ISSUER_SUFFIX):
        raise ValueError("awsidc issuer host %r khong hop le (can *.awsapps.com)" % host)


# derive_idc_region suy region cho endpoint oidc.{region}.amazonaws.com. Uu tien
# idc_region tren callback neu co; neu khong thi scrape env blob (<script id="env">)
# tren trang start cua directory. KHONG mac dinh us-east-1 am tham (goi sai region
# se khong thay directory). Tra region string.
def derive_idc_region(issuer_url, callback_region, proxy_url):
    region = (callback_region or "").strip()
    if region and re.match(r"^[a-z]{2}(-gov)?-[a-z]+-\d$", region):
        return region
    validate_awsidc_issuer(issuer_url)
    start_url = issuer_url.strip().rstrip("/") + "/start/"
    html = http_get_text(start_url, proxy_url, follow_redirects=True)
    m = re.search(r'<script[^>]+id="env"[^>]*>(\{.*?\})</script>', html, re.S)
    if m:
        try:
            ...
            region = ""
    if not region:
        # phu: doi khi oidcApiEndpoint / region xuat hien roi rac trong HTML
        m2 = re.search(r'oidc\.([a-z]{2}(?:-gov)?-[a-z]+-\d)\.(?:api\.aws|amazonaws\.com)', html)
        if m2:
            region = m2.group(1)
    if not region:
        raise RuntimeError("khong suy duoc region IAM Identity Center tu trang start")
    return region


# register_client dang ky OAuth client cong khai voi AWS SSO OIDC va tra ve dict
# (clientId, clientSecret, authorizationEndpoint, tokenEndpoint, ...). Ca public
# client van nhan clientSecret va CreateToken doi lai secret nay.
def register_client(region, issuer_url, redirect_uri, proxy_url):
    host = AWSIDC_OIDC_HOST % region
    payload = {
        "clientName": AWSIDC_CLIENT_NAME,
        "clientType": "public",
        "scopes": AWSIDC_SCOPES,
        "redirectUris": [redirect_uri],
        "grantTypes": ["authorization_code", "refresh_token"],
        "issuerUrl": (issuer_url or "").strip(),
    }
    status, parsed, text = http_post_json(host + "/client/register", payload, None, proxy_url)
    parsed = parsed or {}
    if not (200 <= status < 300) or not parsed.get("clientId"):
        raise RuntimeError("RegisterClient that bai (status %d): %s" % (status, text[:300]))
    return {
        "client_id": parsed.get("clientId", ""),
        "client_secret": parsed.get("clientSecret", ""),
        # dung endpoint AWS tra ve neu co, neu khong dung mac dinh theo region.
        "authorization_endpoint": (parsed.get("authorizationEndpoint")
                                   or (host + "/authorize")).strip(),
        "token_endpoint": (parsed.get("tokenEndpoint") or (host + "/token")).strip(),
    }


# awsidc_authorize_url dung authorize URL auth-code+PKCE cho AWS SSO OIDC. Luu y
# param la "scopes" (so nhieu, space-joined), KHONG co response_mode / login_hint.
def awsidc_authorize_url(auth_endpoint, client_id, redirect_uri, scopes_list, challenge, state):
    q = {
        "response_type": "code",
        "client_id": client_id,
        "redirect_uri": redirect_uri,
...
# exchange_awsidc_code doi authorization code (+ PKCE verifier + clientSecret) lay
# token o token endpoint AWS SSO OIDC. Body la JSON camelCase (khac external_idp
# form-encoded). Tra (access, refresh, expires_in).
def exchange_awsidc_code(token_endpoint, client_id, client_secret, code, verifier,
    ...
    status, parsed, text = http_post_json(token_endpoint, payload, None, proxy_url)
    parsed = parsed or {}
    access = parsed.get("accessToken", "")
    if not (200 <= status < 300) or not access:
        raise RuntimeError("awsidc token exchange that bai (status %d): %s" % (status, text[:300]))
...
class CallbackHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # --- AWS IAM Identity Center leg-1: awsidc descriptor (no code) ---
        # Callback: /signin/callback?login_option=awsidc&issuer_url=https://d-XXXX.awsapps.com/
        # KHONG co client_id -> tu RegisterClient roi 302 sang authorize. Check TRUOC
        # is_descriptor vi issuer_url cung khien is_descriptor=True nhung day la flow khac.
        if path != OAUTH_CALLBACK_PATH and q.get("login_option", "").strip().lower() == "awsidc":
            with state.lock:
                if state.leg2 is not None:
                    return self._empty()
            issuer_url = q.get("issuer_url", "").strip()
            callback_region = q.get("idc_region", "").strip() or q.get("region", "").strip()
            try:
                region = derive_idc_region(issuer_url, callback_region, state.proxy_url)
                redirect_uri = AWSIDC_REDIRECT_URI
                client = register_client(region, issuer_url, redirect_uri, state.proxy_url)
            except Exception as exc:  # noqa: BLE001 - surface setup failure to the user
                self._html(False)
                state.deliver(exc)
                return
            ...
            auth_url = awsidc_authorize_url(
                client["authorization_endpoint"], client["client_id"], redirect_uri,
                AWSIDC_SCOPES, pkce_challenge(verifier), state2)
        ...
        if path == OAUTH_CALLBACK_PATH:
            ...
            if not code:
                return self._empty()
            self._html(True)
            # kind lay tu ctx2 (awsidc) neu co, mac dinh external_idp (Microsoft).
            payload = {"kind": "external_idp", "code": code}
            payload.update(ctx2)
...
def start_listener(portal_state, proxy_url):
    ...
    servers = []
    # Bind v4 voi retry nhe (port 3128 co luc OS chua nha kip giua 2 account).
    last_err = None
    v4 = None
    for _ in range(3):
        try:
...
def build_auth_json(token, region):
    ...
    # External IdP (enterprise) / awsidc refresh material.
    ...
    # awsidc: client_secret bat buoc de refresh; giu cung che do file 0600.
